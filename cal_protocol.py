...
def run(protocol_context):
    ...
    tiprack = [
        protocol_context.load_labware('opentrons_96_tiprack_300ul', 1),
	protocol_context.load_labware('opentrons_96_tiprack_10ul', 4)
    ]

    pip_name = pipette_type.split('_')[-1]

    pipette = protocol_context.load_instrument(
        pipette_type, mount='right', tip_racks=[tiprack[0]])
    pipette2 = protocol_context.load_instrument(
        "p10_single", mount='left', tip_racks=[tiprack[1]])
